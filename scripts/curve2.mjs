...
// book mostly gone, curve boundary prominent — crop the boundary band
const la = await shot('http://localhost:5173', 1950, true)
const ra = await shot('https://www.alternativinc.com', 1950, false)
const crop = (buf) => sharp(buf).extract({ left: 0, top: 250, width: 1920, height: 450 }).resize(960).png().toBuffer()
...
await sharp({ create: { width: 960, height: hh * 2 + 6, channels: 4, background: { r: 255, g: 0, b: 0, alpha: 1 } } })
  .composite([{ input: lc, top: 0, left: 0 }, { input: rc, top: hh + 6, left: 0 }]).png().toFile('recon/final-2/curve2.png')
console.log('done')
await b.close()

...
// ── React Bits "Liquid Ether" — pointer-reactive fluid layer ──────────────────
// Ported verbatim from reactbits.dev (Backgrounds → Liquid Ether): a GPU fluid
// solver (advection → external force → divergence → Poisson pressure) rendered to
// a transparent canvas so it composites OVER whatever sits behind it. In the hero
// it rides ABOVE the CSS aurora (the base weather) and BELOW the book + headline.
//
...
//
// Lifecycle: a built-in IntersectionObserver on the mount pauses the RAF loop when
// the hero scrolls off-screen, and visibilitychange pauses on tab-hide. The hero
// simply does NOT mount this component under prefers-reduced-motion (the static
// aurora gradient remains), so there is no motion to freeze here.
export default function LiquidEther({
  ...
  // final full-screen blit + browser composite, which is where the real cost sat.
  antialias = false,
  maxPixelRatio = 1,
  style = {},
  className = '',

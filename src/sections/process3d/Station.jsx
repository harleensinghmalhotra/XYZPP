...
// ── Machine station geometries (gated on STATION_STYLE) ─────────────────────
// Palette: navy #0F2444/#152444 bodies, gold #B06F14 trim, minimal accents
...
function QualityStation({ api }) {
  ...
  return (
    <group>
      {/* left post */}
      <mesh position={[-ro * 0.7, 1.0, 0]}>
        <boxGeometry args={[0.14, 2.0, 0.12]} />
      ...
      <mesh position={[0, 2.0, 0]}>
        <boxGeometry args={[ro * 1.5, 0.12, 0.15]} />
        <primitive object={metalMat} />
      </mesh>

      {/* scanner head + cone + laser, reused exactly as-is from Quality */}
      ...
      <mesh position={[0, 0.24 + (scanTopY - 0.24) / 2, 0]}>
        <coneGeometry args={[0.42, scanTopY - 0.24, 32, 1, true]} />
      ...
      <mesh position={[0, 0.34, 0]} rotation={[Math.PI / 2, 0, 0]}>
        <cylinderGeometry args={[0.011, 0.011, ro * 1.6, 8]} />
...
function FulfillmentStation() {
  // Wrapping station: mid-height machine body flanking the belt, two side rollers, status light
  const metalMat = useMemo(() => new THREE.MeshStandardMaterial({ color: EKTA.navy, roughness: 0.6, metalness: 0.2 }), [])
  ...
  return (
    <group>
      ...
      <mesh position={[-0.8, 0.45, 0]} rotation={[0, 0, Math.PI / 2]}>
      ...
      {/* status light — emissive sphere on front */}
      <mesh position={[0, 1.2, 0.2]}>
        <sphereGeometry args={[0.08, 16, 16]} />
        <meshStandardMaterial color={EKTA.gold2} emissive={EKTA.gold2} emissiveIntensity={0.8} roughness={0.2} toneMapped={false} />
      </mesh>
    </group>
  )
}

function WarehouseStation() {
  // Background pallet racks: 2 rows of shelf frames with kraft boxes, deep positioning
  const metalMat = useMemo(() => new THREE.MeshStandardMaterial({ color: EKTA.navy, roughness: 0.6, metalness: 0.2 }), [])
  const boxMat = useMemo(() => new THREE.MeshStandardMaterial({ color: EKTA.kraft, roughness: 0.8, metalness: 0.05 }), [])

  return (
    <group position={[0, 0, -2.5]}>
      {/* rack frame left column */}
      <mesh position={[-0.8, 0.8, 0]}>
        <boxGeometry args={[0.08, 1.6, 0.08]} />
        <primitive object={metalMat} />
      </mesh>
      {/* rack frame right column */}
      <mesh position={[0.8, 0.8, 0]}>
        <boxGeometry args={[0.08, 1.6, 0.08]} />
        <primitive object={metalMat} />
      </mesh>
      ...
      {/* kraft boxes on bottom shelf */}
      {[[-0.4, 0.25], [0, 0.25], [0.4, 0.25]].map(([x, y], i) => (
        <mesh key={`box-bottom-${i}`} position={[x, y, 0]}>
          <boxGeometry args={[0.3, 0.5, 0.25]} />
          <primitive object={boxMat} />
        </mesh>
      ))}

      {/* kraft boxes on top shelf */}
      {[[-0.4, 1.35], [0, 1.35], [0.4, 1.35]].map(([x, y], i) => (
        <mesh key={`box-top-${i}`} position={[x, y, 0]}>
          <boxGeometry args={[0.3, 0.5, 0.25]} />
          <primitive object={boxMat} />
        </mesh>
      ))}
...
function ShippingStation() {
  // Truck silhouette: box cab + box trailer, dark navy with emissive headlight
  const metalMat = useMemo(() => new THREE.MeshStandardMaterial({ color: EKTA.navy, roughness: 0.6, metalness: 0.2 }), [])
  ...
  return (
    <group position={[1.2, 0, -1.5]}>
      ...
      {/* truck trailer — large cargo box */}
      <mesh position={[-0.8, 0.45, 0]}>
        <boxGeometry args={[1.2, 0.9, 0.35]} />
        <primitive object={metalMat} />
      </mesh>

      {/* headlight — soft emissive glow facing away */}
      <mesh position={[0.25, 0.6, 0.2]}>
        <sphereGeometry args={[0.1, 16, 16]} />
        <meshStandardMaterial color={EKTA.gold2} emissive={EKTA.gold2} emissiveIntensity={1.0} roughness={0.3} toneMapped={false} />
      </mesh>
    </group>
  )
}

function CoveredStation() {
  // Minimal gold-trimmed bumper/platform edge for the girl's moment
  const trimMat = useMemo(() => new THREE.MeshStandardMaterial({ color: EKTA.gold, emissive: EKTA.gold, emissiveIntensity: 0.2, roughness: 0.35, metalness: 0.75, toneMapped: false }), [])
...
export default function Station({ title, scan, register, swapKey, lang }) {
  ...
  if (STATION_STYLE === 'machines') {
    ...
    return (
      <group>
        {/* plaques float above all machines */}
        <Billboard position={[0, LABEL_Y, 0]}>
          <mesh>
            <planeGeometry args={[0.94, 0.94 / (862 / 649)]} />
            <meshBasicMaterial map={useMemo(() => makeLabelTexture(title), [title])} transparent toneMapped={false} />
          </mesh>
        </Billboard>
        ...
        {swapKey === 'covered' && <CoveredStation />}
      </group>
    )
  }

  // ── Original arch design (COMPLETELY UNTOUCHED) ──────────────────────────────

...
export default function Station({ index, title, variant, accent, glowRef }) {
  ...
  return (
    <group>
      {/* pillars straddling the belt */}
      {[-1, 1].map((s) => (
        <mesh key={s} position={[0, pillarH / 2, s * halfW]} castShadow receiveShadow>
          <boxGeometry args={[0.34, pillarH, 0.34]} />
          <meshStandardMaterial color={EKTA.navy} roughness={0.55} metalness={0.25} />
        </mesh>
      ))}
      ...
      {/* machine block on the far side (variant = machine) */}
      {variant === 'machine' && (
        <mesh position={[0, 0.55, -halfW - 0.55]} castShadow receiveShadow>
